...
#            X[tt][0] = np.sin((tt - signal_time)*rnd_freq/delay)+np.exp(-(tt - signal_time) / delay) * magnitude
    ...
    if params.get('target_output', False):
        ...
        for i in range(Nout):
            for tt in range(len(t)):
                Y[tt][i] = np.exp( -(tt - (signal_time + delay / Nout * (i + 1)))**2 / (2 * width**2)) * magnitude * 3#3
...
#    print "Wrec is: ",rnn.Wrec
#    print "eigen vals are: ",np.linalg.eigvals(rnn.Wrec)
...
#        rnn.Wrec = rnn.Wrec*1.5
        for k in range(1):
            s = 0
#            rnn.Wrec[k+s,:] = rnn.Wrec[k+s,:]*1.5
            rnn.Wrec[:,k+s] = rnn.Wrec[:,k+s]*1
        print "rnn.Wout is: ",rnn.Wout
#        col = 6#12#16 for case 2 (100,10)
##        WrecM = Matrix(rnn.Wrec)
##        WrecMP, WrecMD = WrecM.diagonalize()
#            if((i==j) and (i<10)):
#        asd = np.zeros([Nout, N], dtype=RNN.dtype)
        for i in range(1):
            asd=1
...
##        print "Diagonlized ",WrecMP,WrecMD
##        for j in range(N):
##            WrecMD[j,j] += 0.0*j

##        WrecMM = WrecMP*WrecMD*WrecMP.inv()

##        print "Wrec matrix now is: ",WrecMM
##        rnn.Wrec = np.array(WrecMM).astype(np.float64)

##        print "Wrec now is: ",rnn.Wrec
#/        col = 10
#/        for j in range(int(1.0*len(rnn.Wrec))):
#/            rnn.Wrec[j+0*int(0.8*len(rnn.Wrec)),i] = rnn.Wrec[j+0*int(0.8*len(rnn.Wrec)),i]*1.5#+1.0#rng.uniform(0,0.5)
#            rnn.Wrec[i,j+int(0.8*len(rnn.Wrec))] = rnn.Wrec[i,j+int(0.8*len(rnn.Wrec))]*1.5#+1.0#rng.uniform(0,0.5)
